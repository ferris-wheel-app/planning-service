...
class LifeSchedulerTest extends FunSpec with Matchers {
  ...
  describe("a life-scheduler") {
    ...
    it("should perform a shift, when there are completed laser-donuts") {
      ...
      val tier1 = d.copy(id = 1, tier = 1, status = Planned) :: d.copy(id = 2, tier = 1, status = Complete) :: d.copy(id = 3, tier = 1, status = Complete) :: Nil
      ...
      newTopTier should contain (tier1.head)
